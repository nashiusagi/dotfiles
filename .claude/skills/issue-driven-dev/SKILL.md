...
Vimのキーバインドを追加・変更した場合は、`.vim/myplugin-config/which-key.vim` の `s:keymaps` リストを更新する。

確認対象：
- 新しく追加したキーバインドが `s:keymaps` に含まれているか
- 説明は日本語で記載されているか
...
6. **Vimキーバインドを追加した場合は `\?` のキーマップ一覧も更新する**
